...
def edge_timestamp_feat(edge_timestamp, edge_index, x):
    ...
    # Convert edge_timestamp_sum to a NumPy array before normalization
...
def read_dgraphfin(folder, preprocess=True):
...
class DGraphFin(InMemoryDataset):
    ...
    def process(self):
        data = read_dgraphfin(self.raw_dir, self.preprocess)
